...
for (i in 1:length(unique(ctd$stn))) {
  ...
  date <- paste(ctd$year[ctd$stn == stn], 0 , ctd$month[ctd$stn == stn], ctd$day[ctd$stn == stn], sep = "")
  
  krill_file <- list.files("C:/Users/Lisa/Documents/phd/southern ocean/BROKE-West/Echoview/Extracted data/10x50 integration/", pattern = paste("Transect", ctd$leg[i], sep = ""), full.names = T)
  ...
  krill_38 <- krill_38[krill_38$Time_S > (start_time - 0.05) & krill_38$Time_E < start_time & krill_38$Date_E == date, ]  
  krill_120 <- krill_120[krill_120$Time_S > (start_time - 0.05) & krill_120$Time_E < start_time & krill_120$Date_E == date, ]  
