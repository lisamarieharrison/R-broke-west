...
for (i in 1:nrow(dat.cut)) {
  ...
  w <- which(adcp$date == unique(dat.cut$date[dat.cut$stn == dat.cut$stn[i]]) & (hours(chron(times. = adcp$time, format = "h:m:s") - unique(chron(times. = dat.cut$start.time, format = "h:m:s")[dat.cut$stn == dat.cut$stn[i]])) == 0) & (minutes(chron(times. = adcp$time, format = "h:m:s") - unique(chron(times. = dat.cut$start.time, format = "h:m:s")[dat.cut$stn == dat.cut$stn[i]])) < 20) & ((adcp$depth - dat.cut$profile.depth[i]) < 2))
  if (length(w) > 0) {
