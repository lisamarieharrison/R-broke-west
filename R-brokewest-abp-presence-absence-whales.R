...
for (i in 1:length(broke_transects)) {
  ...
  pred <- read.csv("C:/Users/Lisa/Documents/phd/southern ocean/BROKE-West raw data/Air breathing predator/broke_baleen.csv")
  ...
  pred$dt <- chron(chron(dates. = as.character(pred$date), times. = pred$time, format = list(dates. = "ymd", times. = "h:m:s"), out.format = list(dates. = "d/m/y", times. = "h:m:s")))
  
  if (nrow(pred) != 0) {
    for (j in 1:nrow(pred)) {
      
      points(pred$dt[j], 4, pch = "x", cex = 2, col = "blue")
